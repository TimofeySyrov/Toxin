...
class Header {
  ...
  _handleDocumentClick(event) {
    const { target } = event;
    const isClickOnMenuButton = target === this.menuButton;
    const isClickOnMenu = this.menuContainer.contains(target);
    const isClickOutsideComponent = !isClickOnMenu && !isClickOnMenuButton;
